...
public class KaseyaWSClient_Backup {
	 ...
	 * 
	 */
	protected KaseyaWSClient_Backup(String targetEndPoint) {
	...
	static private Map<String,KaseyaWSClient_Backup> kaseyaWSClients = 
		new HashMap<String,KaseyaWSClient_Backup>();
	
	/**
	 * @return The unique instance of this class.
	 */
	static public KaseyaWSClient_Backup instance(
			String serverName, 
			int serverPort, 
			String sysUsername, 
			String sysPassword,
		...
		KaseyaWSClient_Backup kaseyaWSClient = 
			instance(serverName, 
					serverPort, 
					sysUsername, 
					sysPassword,
	...
	static public KaseyaWSClient_Backup instance(
		...
		KaseyaWSClient_Backup kaseyaWSClient = instance(serverName, serverPort);
	 ...
	 * @return The unique instance of this class.
	 */
	static public KaseyaWSClient_Backup instance(String serverName, int serverPort) {
		...
		KaseyaWSClient_Backup kaseyaWSClient = null;
		...
		if (kaseyaWSClients.containsKey(targetEndPoint)) {
			...
			kaseyaWSClient = new KaseyaWSClient_Backup(targetEndPoint);
...
*/
	...
	public boolean refreshAdminOld(
		...
		try {
			...
			if (enableAdmin(username, password)) {
				...
				//SMS: 5/9/2013
				// Making sure that sessionID is not zero.
				// BigDecimal sessionID = kaseyaAuthenticate(username, password);
				BigDecimal sessionID = getSessionID(username, password);
				if (sessionID == null) {
					DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - refreshAdmin] could not autheticate!");
					DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - refreshAdmin] Ready to get out prematurely!");
					return false;
				}
				String ip = getIP();
				DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - refreshAdmin] " +
			...
			BigDecimal sessionID = getSessionID();
			if (sessionID == null) {
				DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - refreshAdmin] could not autheticate!");
				DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - refreshAdmin] Ready to get out prematurely!");
				return false;
	...
	*/
	...
	private BigDecimal getSessionID(String username, String password) {
		
		DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - getSessionID] Inside!");

		BigDecimal sessionID = null;
		int counter = 0;
		int retry = 5;
		while ((sessionID == null) && (counter < retry)) {
			counter++;
			...
			try {
				...
				DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - getSessionID] could not autheticate! " +
						"username: " + username + " password: " + password);
				DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - getSessionID] Ready to get out prematurely!");
				break;
			}
			
			if (sessionID != null) {
				if (sessionID.intValue() == 0) {
					DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - getSessionID] " +
							"sessionID: " + sessionID);
					sessionID = null;
					if (counter < retry) {
						long waitTime = 5;
						DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - getSessionID] " +
								"wating for " + waitTime + " seconds before retrying!");
						try {
							Thread.currentThread().wait(waitTime * 1000);
		...
		if (sessionID == null) {
			DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - getSessionID] sessionID is null!");
		} else {
			DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - getSessionID] sessionID:" + sessionID);			
		}
		
		DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - getSessionID] Ready to get out!");

		return sessionID;
	}
	
	private BigDecimal getSessionID() {

		DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - getSessionID()] Inside!");
		
		BigDecimal sessionID = getSessionID(null, null);
	...
	public boolean createAdmin(
		...
		// try {
			...
			BigDecimal sessionID = getSessionID();
			if (sessionID == null) {
				DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - createAdmin] could not autheticate!");
				DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - createAdmin] Ready to get out prematurely!");
				return false;
			}
			String ip = getIP();
			...
			*/
			...
			CreateAdminResponseE createAdminResponseE;
			try {
				createAdminResponseE = kaseyaStub.createAdmin(createAdmin);
				CreateAdminResponse createAdminResponse = 
					createAdminResponseE.getCreateAdminResult();
				if (createAdminResponse.getErrorMessage().length() > 0) {
					retVal = false;
			...
			if (promoCode.equals(ITSKSDW) || promoCode.equals(ITSKSDW14)) {
				...
				for (int i=0; i<2; i++) {
					...
					CreateRoleResponseE createRoleResponseE;
					try {
						createRoleResponseE = kaseyaStub.createRole(createRole);
						CreateRoleResponse createRoleResponse = 
							createRoleResponseE.getCreateRoleResult();
						if (createRoleResponse.getErrorMessage().length() > 0) {
							retVal = false;
				...
				for (int i=0; i<3; i++) {
					...
					try {
						addUserToRoleResponseE = kaseyaStub.addUserToRole(addUserToRole);
				...
				*/
				for (int i=0; i<5; i++) {
					...
					AddScopeResponseE addScopeResponseE;
					try {
						addScopeResponseE = kaseyaStub.addScope(addScope);
						AddScopeResponse addScopeResponse = 
							addScopeResponseE.getAddScopeResult();
						if (addScopeResponse.getErrorMessage().length() > 0) {
							retVal = false;
				...
				for (int i=0; i<5; i++) {
					...
					try {
						addUserToScopeResponseE = kaseyaStub.addUserToScope(addUserToScope);
				...
				try {
					addOrgResponseE = kaseyaStub.addOrg(addOrg);
				...
				try {
					addOrgToScopeResponseE = kaseyaStub.addOrgToScope(addOrgToScope);
				...
				for (int i=0; i<machineGroup.length; i++) {
					...
					try {
						createMachineGroupResponseE = kaseyaStub.createMachineGroup(createMachineGroup);
				...
				for (int i=0; i<machineGroup.length; i++) {
					...
					try {
						addMachGroupToScopeResponseE = kaseyaStub.addMachGroupToScope(addMachGroupToScope);
				...
				*/
				...
				for (int i=0; i<5; i++) {
					...
					try {
						addOrgDepartmentResponseE = kaseyaStub.addOrgDepartment(addOrgDepartment);
				...
				*/
				...
				for (int i=0; i<4; i++) {
					for (int j=0; j<naFirstName[i].length; j++) {
						...
						try {
							addOrgDepartmentResponseE = kaseyaStub.addOrgDeptStaff(addOrgDeptStaff);
				...
				for (int i=0; i<=machineGroup.length; i++) {
					...
					try {
						createAgentInstallPackageResponse = kaseyaStub.createAgentInstallPackage(createAgentInstallPackage);
		...
		*/	
		...
		// SMS: 5/9/2013
		// TODO
		// As some of the calls may not be successful because the admin user cannot be enabled for the 
		// first time, etc., then returning false does not mean much!
		retVal = true;
		...
		// try {
			...
			if (enableAdmin(username, password)) {
				...
				try {
					...
					DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - deleteAdmin] could not authenticate! " +
							"username: " + username + " password: " + password);
					DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - deleteAdmin] Ready to get out prematurely!");
					retVal = false;
					return retVal;
				}
				*/
				BigDecimal sessionID = getSessionID(username, password);
				if (sessionID == null) {
					DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - deleteAdmin] could not autheticate!");
					DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - deleteAdmin] Ready to get out prematurely!");
					return false;
				}
				String ip = getIP();
...
*/
	...
	public boolean setAdminPassword(
		...
		try {
			...
			DebugTools.println(DEBUG_LEVEL, "[KaseyaWSClient - setAdminPassword] \n\t" +
	       	...
	       	if (setAdminPasswordResponse.getErrorMessage().length() == 0)
	       		retVal = true;
	...
	public boolean disableAdmin(
		...
		// TODO
		// By passing the disableAdmin so that users can use VSA withour VMs
		/*
		try {
	...
	private BigDecimal kaseyaAuthenticate() throws Exception {
		...
		BigDecimal sessionId = kaseyaAuthenticate(sysUsername, sysPassword);
